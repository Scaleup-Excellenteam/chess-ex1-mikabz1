...
/// @brief Retrieves all legal moves for the specified color.
/// @param color The color of the player whose legal moves are to be retrieved.
/// @return A vector of `Move` objects representing all the legal moves for the given color.
std::vector<Move> Pieces::getAllLegalMoves(COLOR color)const{
    ...
    for(const auto& [coord, piece] : _board){
        // Skip pieces that do not match the specified color
...
/// @brief Makes a move for a piece to the specified destination. If the piece is a pawn and can promote, handles the promotion process.
/// @param destination The coordinate to which the piece should move.
/// @param piece A shared pointer to the piece being moved.
void Pieces::makeMove(const Coordinate & destination,const std::shared_ptr<Piece>& piece) {
    ...
    if (piece->getId() == PAWN){
        ...
        if (pawn->canPromote() && pawn){
            try {
                // Let the user choose the promotion type
                pawn->userPromotionChoice(pawn->getLocation());
                pieceToMove = pawn->getPromotedPiece();// Get the newly promoted piece
                addPiece(pieceToMove);// Add the promoted piece to the board
            }
            catch (WrongPromotionInput& e){
                // Handle invalid promotion input
                std::cout << e.what();
            }
