...
public:
    ...
    // Returns a constant reference to the shared pointer of the promoted piece.
    [[nodiscard]] const std::shared_ptr<Piece> &getPromotedPiece()const;

    // Checks if the current piece is eligible for promotion (e.g., a pawn reaching the last rank).
    bool canPromote();
    // Handles the user’s input to choose which piece to promote to, given the promotion coordinate.
    void userPromotionChoice(const Coordinate &);
private:
